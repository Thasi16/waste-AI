...
def build_mobilenetv3_model(learning_rate=0.001):
    """
    Khởi tạo mô hình MobileNetV3 (Large), kèm theo lớp Data Augmentation ở đầu
    và các lớp phân loại rác ở cuối.
    Trả về mô hình đã được biên dịch (compiled).
    """
    
    # 1. Tải kiến trúc MobileNetV3Large pre-train
    ...
    # LƯU Ý: Khác với V2, MobileNetV3 đã tích hợp sẵn lớp Rescaling ở bên trong nó
    # nên ta KHÔNG cần thêm tf.keras.layers.Rescaling(1./127.5, offset=-1) nữa.
